...
def display_hotels_nodes(state: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalize hotel offers, group by room category, select cheapest per category,
    and store formatted results in state.
    """
    hotels_payload = state.get("hotels_offers", {})
    hotels_list = hotels_payload.get("data", []) if isinstance(hotels_payload, dict) else hotels_payload
    ...
    for hotel_entry in hotels_list:
        ...
        for offer in offers:
            try:
                # Extract category
                category = (
                    offer.get("room", {}).get("typeEstimated", {}).get("category")
                    or offer.get("roomInformation", {}).get("typeEstimated", {}).get("category")
                    or "UNKNOWN_CATEGORY"
                )
                # Extract dates
                ...
                # Extract description
                description = (
                    offer.get("room", {}).get("description", {}).get("text", "")
                    or offer.get("roomInformation", {}).get("description", "")
                )
                # Extract price
                price_info = offer.get("price", {})
                currency = price_info.get("currency", "")
                total_raw = price_info.get("total") or price_info.get("base")
                total_price = Decimal(str(total_raw)) if total_raw is not None else None
                # Extract policies
                ...
                cancellation_policy = (
                    " | ".join(
                        ...
                        for c in policies.get("cancellations", [])
                        if c.get("description", {}).get("text")
                    )
                    or policies.get("refundable", {}).get("cancellationRefund", "")
                )
    ...
    # Group by category and select cheapest per category
    cheapest_per_category = {}
    for offer in formatted_offers:
        if offer["category"] not in cheapest_per_category or (
            offer["total_price"] is not None
            and cheapest_per_category[offer["category"]]["total_price"] > offer["total_price"]
        ):
...
def summarize_hotels_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary_prompt = f"""
        You are a helpful travel assistant.
        Based on the following hotel search results, provide a concise, friendly summary and recommendation.
        Make it brief and conversational, as strings only (no markdown or emojis).

        Search results:
        {json.dumps(formatted_offers, indent=2)}

        Cheapest per category:
        {json.dumps(cheapest_per_category, indent=2)}

        Please provide:
        1. A brief, enthusiastic summary of the search results.
        2. Your recommendation for the best option(s) considering price, timing, and convenience.
        3. Any helpful travel tips or considerations.
        4. Mention any concerns (e.g., non-refundable rates, long stays, etc.).

        Keep it conversational and helpful.
        Start with something like "Great! I found several hotel options for your trip..."
    """
